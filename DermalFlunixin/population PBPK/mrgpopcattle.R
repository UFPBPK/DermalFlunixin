...
# Assuming TOL is a single value
withdrawal_time <- min(subset(data_tissue, CL99 <= 0.125 & Time1 > 0.2) %>% dplyr::select(Time1))
...
p1<-ggplot(data_tissue, aes(Time1)) + 
  ...
  geom_vline(aes(xintercept = withdrawal_time), 
             ...
             show.legend = F)+
  annotate("text", x = withdrawal_time, y = 0.125, label = paste("Liver Withdrawal Interval:",round(withdrawal_time, 2),"Days"),hjust = -0.1, vjust=-0.5,angle = 0, size = 6)
...
withdrawal_time2 <- min(subset(data_tissue, CM99 <= 0.025 & Time1 > 0.2) %>% dplyr::select(Time1))
...
p3<-ggplot(data_tissue, aes(Time1)) + 
  ...
  ylab(expression(paste('Concentration (', mu, 'g/mL)'))) + 
  geom_line(aes(y = 0.025), color = 'black', size = 0.5, linetype = 'twodash', show.legend = FALSE) + 
  scale_y_log10() + 
  theme_bw() + 
  theme(axis.text = element_text(size = 16), legend.title = element_blank()) + 
  geom_vline(aes(xintercept = min(subset(data_tissue, CM99 <= 0.025 & Time1 > 0.2) %>% dplyr::select(Time1))), size = 1, color = "red", linetype = 2, show.legend = FALSE) + 
  annotate("text", x = withdrawal_time2, y = 0.025, label = paste("Muscle Withdrawal Interval:", round(withdrawal_time2, 2), "Days"), hjust = -0.1, vjust = -0.5, angle = 0, size = 6)
p3
